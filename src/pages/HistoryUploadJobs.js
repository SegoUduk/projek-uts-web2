...
function HistoryUploadJobs() {
      ...
      applicants: [
        {
          ...
          status: "pending", // Bisa 'pending', 'accepted', 'rejected'
        },
        {
          id: 2,
          name: "Budi",
  ...
  const handleAcceptApplicant = (jobId, applicantId) => {
    // Update status pelamar jadi 'accepted'
    ...
    setSelectedApplicants(updatedJobs);
  };

  const handleRejectApplicant = (jobId, applicantId) => {
    // Update status pelamar jadi 'rejected'
    const updatedJobs = jobsUploaded.map((job) => {
      if (job.id === jobId) {
        const updatedApplicants = job.applicants.map((applicant) => {
          if (applicant.id === applicantId) {
                ...
                {selectedJob.applicants.map((applicant) => (
                  <li key={applicant.id}>
                    <div className="applicant-detail">
                      <img src={applicant.profile} alt={applicant.name} className="applicant-profile" />
                      <div>
